...
// custom
$dt_color = $dt_color_opacity = $dt_text_scheme = '';
// end custom

$output = $after_output = '';
$atts = vc_map_get_attributes( $this->getShortcode(), $atts );

// custom adding new items to shortcode array
$atts[] = $dt_color;
$atts[] = $dt_color_opacity;
$atts[] = $dt_text_scheme;
// end custom
...
if ( ! empty( $parallax ) ) {
	...
	if ( false !== strpos( $parallax, 'fade' ) ) {
		...
		//custom
		$css_classes[] = $dt_text_scheme;
		if(!empty($dt_color)) {
		$wrapper_attributes[] = 'style="background-color: rgba(' . $rgbcolor[0] . ', ' . $rgbcolor[1] . ', ' . $rgbcolor[2] . ', '.$cop.');"';	
		}
		//end custom
